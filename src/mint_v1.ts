...
export async function mintEditionFromMaser(
  ...
  const masterData = (new programs.metadata.MasterEdition(masterPDA, masterInfo)).data;
  ...
  const tokenAccount = await Token.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, masterEditionMint, wallet.publicKey);

  // ------------------ pdas + final ix
  const metadataPDA = await programs.metadata.Metadata.getPDA(mint.publicKey);
  //NOTE: requirest master's mint, not edition's mint
  const editionMarker = await programs.metadata.EditionMarker.getPDA(masterEditionMint, editionValue);
  const editionPDA = await programs.metadata.Edition.getPDA(mint.publicKey);

  const newEditionFromMasterTx = new programs.metadata.MintNewEditionFromMasterEditionViaToken(
    {feePayer: wallet.publicKey},
    {
