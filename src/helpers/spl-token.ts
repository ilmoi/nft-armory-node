...
export async function deserializeToken(mintPubkey: PublicKey): Promise<Token> {
  ...
  return new Token(CONN, mintPubkey, TOKEN_PROGRAM_ID, tempKeypair);
}

export async function deserializeTokenAccount(mintPubkey: PublicKey, tokenAccountPubkey: PublicKey): Promise<AccountInfo> {
...
export async function getTokenBalance(tokenAccountPubkey: PublicKey): Promise<number> {
  const balance = await CONN.getTokenAccountBalance(tokenAccountPubkey);
